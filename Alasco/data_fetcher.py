...
class DataFetcher:
    ...
    def get_json(
        self,
        url: str,
        filters: Optional[tuple[str, str, list]] = None,
        verbose: bool = False,
    ) -> list | RuntimeError:
        """
        ...
        try:
            ...
            if nextPageUrl is not None:
                ...
                if isinstance(nextPagesJSON, list):
    ...
    def get_df(
        ...
        def fetch_and_convert(
            url: str, filters: Optional[tuple[str, str, list]]
        ) -> Optional[pd.DataFrame]:
            """
            Fetches JSON data from a URL with specified filters and converts it to a pandas DataFrame.

            Args:
                url (str): The API endpoint URL to fetch data from.
                filters (list): A list of filter parameters to apply to the API request.

            Returns:
                Optional[pd.DataFrame]: A pandas DataFrame containing the data if the response is a list,
                                        or None if the response is not a list.
            """
        ...
        if filters is not None:
            ...
            if isinstance(filter_values, (list, tuple)):
                ...
                if len(filter_values) > chunk_size:
                    filter_chunks = self.utils.split_list(
                        input_list=filter_values, chunk_size=chunk_size
                    )
                    dfs = []
                    ...
                    for index, chunk in enumerate(filter_chunks):
                        if self.verbose:
                            print(
                                f"API call Nr. {index + 1} from {len(filter_chunks)} to url: {url}"
                            )
    ...
    def get_projects(
        self, property_ids: list | None = None, project_name: str | None = None
    ) -> Optional[pd.DataFrame]:
        """
        ...
        if project_name is not None:
            ...
            if self.verbose:
                print(
                    f"Fetching project data for projects which name contains: {project_name}"
                )
    ...
    def get_properties(
        self, property_ids: list | None = None, property_name: str | None = None
    ) -> Optional[pd.DataFrame]:
        """
        ...
        if property_ids is not None:
            ...
            if self.verbose:
                print(
                    f"Fetching properties data for property_ids: {property_ids[:3]} ..."
                )
        elif property_name is not None:
            filters = ("name", "contains", [property_name])
            if self.verbose:
                print(
                    f"Fetching properties data for property which name contains: {property_name}"
                )
        else:
            if self.verbose:
    ...
    def get_reporting(self, project_ids: list) -> Optional[pd.DataFrame]:
        """
        ...
        df_reporting = self.get_df(
            url=self.URL_REPORTING, filters=filters, chunk_size=10
        )
        return df_reporting

    def get_contractors(
        self, contractor_ids: list | None = None, contractor_name: str | None = None
    ) -> Optional[pd.DataFrame]:
        """
    ...
    def get_contracts(
        ...
        if contract_number is not None:
            ...
            if self.verbose:
                print(
                    f"Fetching contracts with contractor ids in: {contractor_ids[:3]} ..."
                )
        elif contract_unit_ids is not None:
            filters = ("contract_unit", "in", contract_unit_ids)
            if self.verbose:
                print(
                    f"Fetching contracts with contract unit ids in: {contract_unit_ids[:3]} ..."
                )
        else:
            filters = None
            if self.verbose:
    ...
    def get_contracting_entities(self, name: str | None = None):
        ...
        df_contracting_entities = self.get_df(
            url=self.URL_CONTRACTING_ENTITIES, filters=filters
        )

        return df_contracting_entities

    def get_contract_units(
        self, contract_unit_ids: list | None = None, project_ids: list | None = None
    ) -> Optional[pd.DataFrame]:
        """
        ...
        if contract_unit_ids is not None:
            ...
            if self.verbose:
                print(
                    f"Fetching contract units with ids in: {contract_unit_ids[:3]}..."
                )
        elif project_ids is not None:
            filters = ("project", "in", project_ids)
            if self.verbose:
                print(
                    f"Fetching contract units with project ids in: {project_ids[:3]} ..."
                )
        else:
            filters = None
            if self.verbose:
    ...
    def get_invoices(
        self, invoice_ids: list | None = None, contract_ids: list | None = None
    ) -> Optional[pd.DataFrame]:
        """
    ...
    def get_change_orders(
        self, change_order_ids: list | None = None, contract_ids: list | None = None
    ) -> Optional[pd.DataFrame]:
        """
        ...
        if change_order_ids is not None:
            ...
            if self.verbose:
                print(
                    f"Fetching change orders belonging to contract ids in: {contract_ids[:3]} ..."
                )
            filters = ("contract", "in", contract_ids)
        else:
            raise ValueError(
                "Please provide either a list of change order ids or a list of contract ids."
            )
    ...
    def get_all_df(
        self, property_name: str | None = None, project_name: str | None = None
    ):
        """
        ...
        property_ids = (
            self.utils.get_ids(df_properties) if df_properties is not None else []
        )

        # Fetch projects and their IDs
        df_projects = self.get_projects(
            property_ids=property_ids, project_name=project_name
        )
        project_ids = self.utils.get_ids(df_projects) if df_projects is not None else []

        # Fetch contract units and their IDs
        df_contract_units = self.get_contract_units(project_ids=project_ids)
        contract_unit_ids = (
            self.utils.get_ids(df_contract_units)
            if df_contract_units is not None
            else []
        )
        ...
        contract_ids = (
            self.utils.get_ids(df_contracts) if df_contracts is not None else []
        )

        # Fetch contractor IDs from contracts
        contractor_ids = (
            df_contracts["contractor"].tolist() if df_contracts is not None else []
        )
    ...
    def get_contract_documents(self, contract_ids: list) -> pd.DataFrame:
        ...
        if self.verbose:
            print(
                f"Getting contract documents for {len(contract_ids)} documents with ids : {contract_ids[:3]} ..."
            )

        urls = [
            self.utils._prepare_url_get_contract_documents(contract_id)
            for contract_id in contract_ids
        ]
        collection = []
        for url in urls:
            doc = self.get_df(url=url)
            collection.append(doc) if doc is not None else None
    ...
    def get_change_order_documents(self, change_order_ids: list) -> pd.DataFrame:
        ...
        if self.verbose:
            print(
                f"Getting change order documents for {len(change_order_ids)} documents with ids : {change_order_ids[:3]} ..."
            )
        urls = [
            self.utils._prepare_url_get_change_order_documents(change_order_id)
    ...
    def get_invoice_documents(self, invoice_ids: list) -> pd.DataFrame:
        ...
        if self.verbose:
            print(
                f"Getting invoice documents for {len(invoice_ids)} documents with ids : {invoice_ids[:3]} ..."
            )

        urls = [
            self.utils._prepare_url_get_invoice_documents(invoice_id)
